var rule = {
	...
	searchUrl: '/p/0/s/https://www.meijumi.xyz/?s=**',
	searchable:2,
	quickSearch:0,
	filterable:0,
	headers:{
...
items.forEach(it => {
	let burl = rule.homeUrl.replace("https://www.meijumi.xyz/","") + pd(it, 'a&&href').replace(rule.host, "https://www.meijumi.xyz");
	d.push({
...
if (typeof items !== "undefined") {
	items.forEach(it => {
		let burl = rule.homeUrl.replace("https://www.meijumi.xyz/","") + pd(it, 'header a&&href').replace(rule.host, "https://www.meijumi.xyz");
		d.push({
